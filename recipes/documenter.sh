#!/usr/bin/env bash
# Recipe: Document cards — generate/update docs, open PR
set -euo pipefail
...
log_info "Documenter: checking $RECIPE_DOCUMENTER_FROM lane..."

ISSUE_IDS=$(board_get_cards_in_status "$RECIPE_DOCUMENTER_FROM" "$MAX_CARDS_DOCUMENTER")

if [[ -z "$ISSUE_IDS" ]]; then
  log_info "No cards in $RECIPE_DOCUMENTER_FROM. Nothing to document."
  exit 0
fi
...
PREOF
  ...
  PR_URL=$("$GH_CMD" pr create \
    ...
    if [[ -n "$RECIPE_DOCUMENTER_TO" ]]; then
      local_transition="TRANSITION_TO_${RECIPE_DOCUMENTER_TO}"
  ...
  if [[ -n "$RECIPE_DOCUMENTER_TO" ]]; then
    local_transition="TRANSITION_TO_${RECIPE_DOCUMENTER_TO}"
    board_transition "$ISSUE_KEY" "${!local_transition}"
    log_info "Done: $ISSUE_KEY documented and moved to $RECIPE_DOCUMENTER_TO"
  else
