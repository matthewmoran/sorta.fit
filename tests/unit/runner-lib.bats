...
# ── runner_transition event logging ─────────────────────────────────────────

@test "runner_transition: emits card_transitioned event with transition mapping" {
  ...
  run node -e "
    ...
    const found = lines.some(l => {
      ...
      return e.event === 'card_transitioned' &&
             e.data && e.data.card_key === 'TEST-1' &&
             e.data.target_status === '10070' &&
             e.data.transition_configured === 'true';
    });
...
@test "runner_transition: emits card_transitioned event with empty target_status" {
  export EVENT_LOGGING="on"
  export RUNNER_NAME="test"
  runner_transition "TEST-2" "" "refined"
  local event_file="$SORTA_ROOT/.sorta/events.jsonl"
  [[ -f "$event_file" ]]
  run node -e "
    const fs = require('fs');
    ...
    const found = lines.some(l => {
      const e = JSON.parse(l);
      return e.event === 'card_transitioned' &&
             e.data && e.data.transition_configured === 'false';
    });
    if (!found) process.exit(1);
  " "$event_file"
  assert_success
